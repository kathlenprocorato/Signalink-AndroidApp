...
class GestureRecognizerHelper(
    ...
    fun recognizeLiveStream(imageProxy: ImageProxy, mirrored: Boolean) {
    ...
    fun recognizeVideoFile(
        ...
        for (i in 0..numberOfFrameToRead) {
            ...
            retriever.getFrameAtTime(timestampMs * 1000, MediaMetadataRetriever.OPTION_CLOSEST)?.let { frame ->
                val argb8888Frame = if (frame.config == Bitmap.Config.ARGB_8888) frame else frame.copy(Bitmap.Config.ARGB_8888, false)
                val mpImage = BitmapImageBuilder(argb8888Frame).build()

                gestureRecognizer?.recognizeForVideo(mpImage, timestampMs)?.let { recognizerResult ->
                    resultList.add(recognizerResult)
                } ?: run {
                    didErrorOccurred = true
                    gestureRecognizerListener?.onError("ResultBundle could not be returned in recognizeVideoFile")
                }
            } ?: run {
                didErrorOccurred = true
                gestureRecognizerListener?.onError("Frame at specified time could not be retrieved when recognition in video.")
            }
        }
    ...
    fun recognizeImage(image: Bitmap): ResultBundle? {
        ...
        gestureRecognizer?.recognize(mpImage)?.also { recognizerResult ->
            ...
            return ResultBundle(listOf(recognizerResult), inferenceTimeMs, image.height, image.width)
        }

        gestureRecognizerListener?.onError("Gesture Recognizer failed to recognize.")
        return null
    }
    ...
    companion object {
        ...
        private const val MP_RECOGNIZER_TASK = "improved_fingerspelling.task"
